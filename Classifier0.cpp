...
void Classifier0::train(InputChar* inputArr, int length) {
...
void Classifier0::classify(const InputChar& input) {
	inputChar = input;
	for (int i = 0; i < 10; ++i) {
		outputDouble[i] = 0.0;
	}
	for (int i = 0; i < 26; ++i) {
		for (int j = 0; j < 10; ++j) {
			outputDouble[j] += inputChar[i] * weight[i][j];
		}
	}
	for (int i = 0; i < 10; ++i) {
		if (outputDouble[i] >= 0) {
...
void Classifier0::classify(const InputChar& input, int* _output) {
	double outputD[10];
	for (int i = 0; i < 10; ++i) {
		outputD[i] = 0.0;
	}
